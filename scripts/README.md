...
- `verify-cocos-controller.mjs`: executes the Cocos main scene controller with a mocked backend and checks core screen, business, settlement, upgrade, task, and snapshot recovery flows.
- `verify-cocos-project.mjs`: checks Cocos project metadata, portrait design resolution, per-screen scene blueprint alignment, scene-wiring alignment, and generated-directory ignore rules.
