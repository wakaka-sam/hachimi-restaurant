...
test('runtime client sources do not use drawing or filter effects for art', async () => {
  ...
  const forbidden = /canvas|<svg|drawImage|getContext|createElement\(['"]canvas|Canvas|linear-gradient|radial-gradient|conic-gradient|box-shadow|text-shadow|border-radius\s*:|animation\s*:|@keyframes|scale\s*\(|filter\s*:|opacity\s*:/i;
